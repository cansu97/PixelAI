...
class PixelAI:
    def __init__(self, env=0):
        ...
        self.s_v = np.empty((1,1,self.height, self.width)) # Currently observed visual sensation
        self.g_mu = np.empty((1,1,self.height, self.width)) # Predicted visual sensation
        self.pred_error = np.empty((1,1,self.height, self.width))
        ...
        if env:
            self.model_path = 'Conv_decoder_model/conv_decoder_real'
        else:
    ...
    def reset(self, params, start_mu, attractor_pos = None):
        ...
        self.s_v = np.empty((1,1,self.height, self.width))
        ...
        if np.all(attractor_pos) != None: # Active inference activated
            ...
            self.attr_error = np.empty((1, 1, self.height, self.width))
            self.a = np.zeros((1, 4))
            self.a_thres = 2.0 * (np.pi / 180) / self.dt # Maximum 2 degrees change per timestep
        else: # Perceptual inference
    ...
    def set_visual_sensation(self, cv_image):
        ...
        self.s_v = camera_im.reshape([1, 1, camera_im.shape[0], camera_im.shape[1]])
    ...
    def visual_forward(self):
        input = torch.FloatTensor(minmax_normalization(self.mu, self.data_max, self.data_min)).to(device)
    ...
    def get_dF_dmu_dyn(self, input, out):
        ...
        return input.grad.cpu().data.numpy() # mu_dot_dyn (1x4 vector)
